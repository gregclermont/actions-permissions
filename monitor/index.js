  ...
  try {
    ...
    if (!!core.getState("isPost")) {
      ...
      if (config.create_artifact) {
        ...
        if (fs.existsSync(debugLog) && fs.statSync(debugLog).size > 0) {
          await new DefaultArtifactClient().uploadArtifact(
      ...
      for (const envVar of [
        ...
        if (process.env[envVar]) {
          hosts.add(new URL(process.env[envVar]).hostname.toLowerCase());
        }
      }
      ...
      const proc = spawn("bash", ["-e", "setup.sh"], {
        cwd: `${__dirname}/..`,
      });
      ...
      proc.stderr.on("data", (output) => {
        ...
        if (/error/i.test(msg)) {
          core.setFailed(`Setup error: ${msg}`);
          process.exit(1);
        }
      });
      proc.on("exit", (code) => {
